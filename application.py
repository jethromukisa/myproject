...
def password():
    ...
    if request.method == "POST":
        ...
        rows=db.execute("SELECT name FROM clients WHERE name=:name",name=request.form.get("name"))
...
def history():
    ...
    dates = db.execute("SELECT date FROM appointments WHERE clientId = :id ORDER BY date(date) DESC",id = session["user_id"])
